    ...
    <div class="messages-container" ref="messagesContainerRef">
      <div v-if="messages.length === 0" class="messages-empty">
        <div class="empty-state">
          ...
          <h2 class="empty-title">任务助手</h2>
          <p class="empty-desc">询问关于此任务的任何问题</p>
        </div>
      </div>
...
// 使用 taskId 作为 sessionId
const sessionId = computed(() => `task-${props.task.id}`)
...
watch(() => props.task.id, async (id) => {
  if (id) {
    await chatStore.loadSessionMessages(sessionId.value)
    initSessionModel()
    scrollToBottom()
  }
}, { immediate: true })
